...
], function ()
{
    ...
    return {
        ...
        checkPropertiesAttachFC: async function (that)
        {
            ...
            {
                await that.messageBoxError(that.getResourceBundle().getText("attachmentNameTooLong"));
                return false;
            };
            if (this.getPropertyAttachFC(that.getConstantBase().getConstants().FILE_NAME_SIZE_EXCEEDED,that) === false)
            {
                await that.messageBoxError(that.getResourceBundle().getText("attachmentSizeExceeded"));
                return false;
            };
            if (this.getPropertyAttachFC(that.getConstantBase().getConstants().FILE_TYPE_MISMATCH_EXCEEDED,that) === false)
            {
                await that.messageBoxError(that.getResourceBundle().getText("attachmentWrongType"));
                return false;
            };
