...
sap.ui.define([
...
], function (Message, MessageType, ValueState, ValidateException) {
    "use strict";
     ...
     */
    Validator.prototype.validate = function (oControl,bRemoveMessage) {
        this._isValid = true;
        if (bRemoveMessage === true) {
     ...
     Validator.prototype.validateFilterBar = function (oControl) {
        this._isValid = true;
        sap.ui.getCore().getMessageManager().removeAllMessages();
        this._validate(oControl);
        return this.isValid();
    ...
    Validator.prototype._validate = function (oControl) {
        var aPossibleAggregations = ["header","filterGroupItems","items", "content", "form", "formContainers", "formElements", "fields", "sections", "subSections", "_grid", "cells", "_page","_dynamicPage"],
        ...
        if (( oControl instanceof sap.ui.core.Control 
            ...
            for (i = 0; i < aValidateProperties.length; i += 1) {
                if (oControl.getBinding(aValidateProperties[i]) 
                    ...
                    if(editable) {
	                    try { // try validating the bound value
	                        ...
	                        //for Date nullable value in Date type
	                        if (oControlBinding.getType().sName === "Date" 
	                        	&& oControl.getRequired 
                        		&& oControl.getRequired() === true && oExternalValue === "") {
	                        		
	                        			throw new ValidateException("Zadejte datum");
	                        }
                    ...
                    try {
                        ...
                        if  (typeof oControl.getBinding(aValidateProperties[i]) === "object") {
                        	//pokračuje se
                        } else {
                        	continue;
                        }
                        ...
                        if (!oExternalValue || oExternalValue==="") {
                            this._isValid = false;
                           var oMessage = "Vyplňte povinná pole!";
                ...
                }  else if (oControl.mProperties.hasOwnProperty("selectedKey") && oControl.getLabels && oControl.getLabels().length > 0) { //Select pro FORMS
					if (oControl.getLabels && oControl.getLabels().length > 0 && oControl.getLabels()[0].mProperties.hasOwnProperty("required")) {
						if (oControl.getLabels()[0].getProperty("required") === false) {
							continue;
						} 
						try {
							oControlBinding = oControl.getBinding(aValidateProperties[i]);
							...
							if (!oExternalValue || oExternalValue === "") {
								this._isValid = false;
								var oMessage = "Vyplňte povinná pole: " + oControl.getLabels()[0].getProperty("text");
								oControl.setValueState(ValueState.Error, oMessage);
                ...
                }  else if (oControl.mProperties.hasOwnProperty("selectedKey")) { //Select
					if (oControl.getParent().getItems().length > 0  && oControl.getParent().getItems()[0].mProperties.hasOwnProperty("required") === true) {
						if (oControl.getParent().getItems()[0].getProperty("required") === false) {
							continue;
						} 
						try {
							oControlBinding = oControl.getBinding(aValidateProperties[i]);
							...
							if (!oExternalValue || oExternalValue === "") {
								this._isValid = false;
								var oMessage = "Vyplňte povinná pole: " + oControl.getParent().getItems()[0].getProperty("text");
								oControl.setValueState(ValueState.Error, oMessage);
